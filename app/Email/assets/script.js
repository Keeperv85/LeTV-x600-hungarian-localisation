 ...
 */
var TRANSFORM_MINIMUM_EFFECTIVE_RATIO = 0.9;
...
function normalizeElementWidths(elements) {
    ...
    for (i = 0; i < elements.length; i++) {
        ...
        if (NORMALIZE_MESSAGE_WIDTHS && newZoom < 1) {
...
function transformContent(el, docWidth, elWidth) {
    ...
    if (!done) {
        ...
        touched = addClassToElementsExt(nodes, shouldMungeTable, "munged", el, docWidth, elWidth,
            actionLog);
        if (touched) {
            newWidth = el.scrollWidth;
...
function markSubTables(node) {
    var nodes;
    var i, len;
    var child;

    nodes = node.querySelectorAll("table");
    for (i = 0, len = nodes.length; i < len; i ++) {
        child = nodes[i];
        child.leParentScale = "true";
    }
...
function addClassToElementsExt(nodes, conditionFn, classToAdd, el, docWidth, elWidth, actionLog) {
    var i, len;
    var node;
    var added = false;
    var scaleWidth;
    for (i = 0, len = nodes.length; i < len; i++) {
        ...
        if (!conditionFn || conditionFn(node)) {
            ...
            if (node.scrollWidth > docWidth && node.leParentScale != "true") {
	             saveStyleProperty(node, "webkitTransformOrigin", actionLog);
                 saveStyleProperty(node, "webkitTransform", actionLog);
                 saveStyleProperty(node, "marginTop", actionLog);

                 elWidth = node.scrollWidth;
                 node.style.webkitTransformOrigin = "left bottom";
                 scaleWidth = docWidth;
                 if (node.getBoundingClientRect().left > BODY_MARGIN) {
                     scaleWidth = docWidth - node.getBoundingClientRect().left + BODY_MARGIN;
                 }

                 node.style.webkitTransform = "scale(" + (scaleWidth/elWidth) + ")";
                 node.style.marginTop = -node.scrollHeight * (1 - scaleWidth /elWidth)  + "px";
...
function transformBlockElements(nodes, docWidth, actionLog) {
    ...
    for (i = 0, len = nodes.length; i < len; i++) {
        ...
        w = wStr.slice(0, index);
        if (index >= 0 && w + node.getBoundingClientRect().left> docWidth) {
            scaleWidth = docWidth - node.getBoundingClientRect().left + BODY_MARGIN;
            if (scaleWidth <= 0) continue;
            ...
            node.style.minWidth = "";
            //node.style.maxWidth = scaleWidth + "px";
            touched = true;
        }
    }
    return touched;
...
function transformImages(nodes, docWidth, actionLog) {
    ...
    for (i = 0, len = nodes.length; i < len; i++) {
        ...
        if (w + node.getBoundingClientRect().left > docWidth && node.leParentScale != "true") {
            scaleWidth = docWidth - node.getBoundingClientRect().left + BODY_MARGIN;
            if (scaleWidth <= 0) continue;
            saveStyleProperty(node, "maxWidth", actionLog);
            saveStyleProperty(node, "width", actionLog);
            ...
            node.style.maxWidth = scaleWidth + "px";
            node.style.width = scaleWidth + "px";
...
function hideUnsafeImages(msgContentDivs) {
    ...
    for (i = 0, msgContentCount = msgContentDivs.length; i < msgContentCount; i++) {
        ...
        for (j = 0, imgCount = images.length; j < imgCount; j++) {
            ...
            attachImageLoadListener(image, j);
...
function attachImageLoadListener(imageElement, position) {
    ...
    /// EUI[GUI.XUN.HU]: [BUG.MODIFY] @{
    // used to click inline view image and long click save image
    /*imageElement.onclick = function() {
            console.log("gnod:onclick:src=" + this.src + ", cid=" + this.getAttribute('data-cid'));
            window.mail.onInlineImgClick(this.src, this.getAttribute("data-cid"));
        }*/
        var longpress = false;
            var presstimer = null;
            var longtarget = null;
            ...
            var click = function(e) {
                ...
                //alert("press" + position);
                /// EUI[GUI.XUN.HU]: [BUG.ADD] @{
                //  used to check if start with "http://"
                if(isStartWithHttp(this.src)) return false;
                /// @}

        		window.mail.onInlineImgClick(this.src, this.getAttribute("data-cid"), position);
            };
            ...
            var start = function(e) {
                ...
                var cid = this.getAttribute("data-cid");
           	    presstimer = setTimeout(function() {
                    longpress = true;
                    window.mail.onInlineImgLongClick(imgSrc, cid, position);
                }, 500);
                return false;
            };
        	imageElement.addEventListener("mousedown", start);
        	imageElement.addEventListener("touchstart", start);
        	imageElement.addEventListener("click", click);
        	imageElement.addEventListener("mouseout", cancel);
        	imageElement.addEventListener("touchend", cancel);
        	imageElement.addEventListener("touchleave", cancel);
        	imageElement.addEventListener("touchcancel", cancel);
    /// @}
}

/// EUI[GUI.XUN.HU]: [BUG.ADD] @{
//  used to check if start with "http://"
function isStartWithHttp(str) {
...
/*
    document.onscroll = function() {
        var scale = getScale();
        var bottomHeight = mBottomPadding / scale;
        var topHeight = mTopPadding / scale;
        console.log("zoom=" + getScale() + ", " + bottomHeight + ", " + topHeight + ":" + mBottomPadding + ", " + mTopPadding);

        if (currentScale != scale) {
            adjustTopHeight(topHeight);
            adjustBottomHeight(bottomHeight);
            currentScale = scale;
        }
    }
*/
...
function updateSrc(cid, src) {
    var objs = document.getElementsByTagName("img");
    for (var i = 0; i < objs.length; i++) {
        if (objs[i].src.substr("cid:".length) == cid || objs[i].getAttribute("data-cid") == cid) {
            objs[i].src = src;
            objs[i].setAttribute("data-cid", cid);
        }
    }
}

function adjustTopHeight(h) {
    var headNode = document.querySelector(".mail-message-header");

    headNode.style.height = h + "px";
    console.log("adjustTopHeight=" + headNode.style.height);
}

function adjustBottomHeight(h) {
    var headNode = document.getElementById("conversation-footer");
...
function setBodyTopPadding(h) {
    mTopPadding = parseInt(h);
    console.log("padding:" + mTopPadding);
}

function setBodyBottomPadding(h) {
    mBottomPadding = parseInt(h);
}
...
/// M: [BUG.MARK] fix bug: when first click inline is not response @{
// LEUI guixunhu
/*function setImageClickListener() {
    var imgs = document.getElementsByTagName("img");
    for (var i = 0; i < imgs.length; i ++) {
        imgs[i].onclick = function() {
            window.mail.onInlineImgClick(this.src,
